"""End-to-end pipeline: load data, train, evaluate, demo inference."""

import argparse

from codeuctivity.data import build_tokenizer, load_data, preprocess_dataset
from codeuctivity.inference import Summarizer
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Full training + eval pipeline")
    parser.add_argument("--config", default="configs/trial.yaml", help="Path to YAML config")
    ...
    training_args = build_training_args(output_dir=cfg["model"]["output_dir"], **cfg["training"])
    trainer = run_training(model, tokenizer, train_ds, eval_ds, training_args)
    tokenizer.save_pretrained(cfg["model"]["output_dir"])
    print(f"Training complete. Saved to {cfg['model']['output_dir']}")

    summarizer = Summarizer(model, tokenizer, max_output_len=cfg["data"]["max_output_len"])
    demo = "def factorial(n): return 1 if n==0 else n*factorial(n-1)"
    print(f"\nDemo inference:\n  Input:  {demo}\n  Output: {summarizer.summarize(demo)}")
